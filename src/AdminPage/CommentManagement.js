  ...
  async function getComments() {
    try {
      const response = await axios({
        method: "get",
        url: `${process.env.REACT_APP_API_URL}get-all-comments`,
      });
  ...
  const handleComments = {
    onDeleteComment: async (comment_id, details_product_id) => {
      if (window.confirm("Bạn có chắc muốn xóa đánh giá này không?") == true) {
        try {
          const response = await axios({
            method: "post",
            url: `${process.env.REACT_APP_API_URL}delete-comment`,
            data: {
    ...
    onConfirmComment: async (comment_id, details_product_id) => {
      try {
        const response = await axios({
          method: "post",
          url: `${process.env.REACT_APP_API_URL}confirm-comment`,
          data: {
            comment_id: comment_id,
            details_product_id: details_product_id,
          },
    ...
    onSearchComment: async (details_product_id) => {
      try {
        const response = await axios({
          method: "post",
          url: `${process.env.REACT_APP_API_URL}search-comments`,
          data: {
            details_product_id: details_product_id,
          },
        });
