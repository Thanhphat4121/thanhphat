...
function ProductManagement() {
  ...
  async function getBills() {
    try {
      const response = await axios({
        method: "get",
        url: `${process.env.REACT_APP_API_URL}get-bills`,
      });
  ...
  async function decreaseStorage(amount, details_product_id) {
    try {
      const response = await axios({
        method: "post",
        url: `${process.env.REACT_APP_API_URL}update-storage`,
        data: {
  ...
  const searchBills = async (bill_id) => {
    try {
      const response = await axios({
        method: "post",
        url: `${process.env.REACT_APP_API_URL}search-bills`,
        data: {
          bill_id: bill_id,
        },
      });
  ...
  const handleBills = {
    ...
    onConfirmBill: async (bill_id) => {
      try {
        const response = await axios({
          method: "post",
          url: `${process.env.REACT_APP_API_URL}confirm-bill`,
          data: {
            bill_id: bill_id,
          },
        });
    ...
    onDeleteBill: async (bill_id) => {
      if (window.confirm("Bạn có chắc muốn xóa bill này không?") == true) {
        try {
          const response = await axios({
            method: "post",
            url: `${process.env.REACT_APP_API_URL}delete-bill`,
            data: {
              bill_id: bill_id,
            },
          });
