...
function ResetPasswordPage() {
  ...
  const getMail = async (token) => {
    try {
      const response = await axios({
        method: "post",
        url: `${process.env.REACT_APP_API_URL}get-reset-mail`,
        data: {
          token: token,
        },
      });
  ...
  const changePassword = async (email, password) => {
    ...
    try {
      const response = await axios({
        method: "post",
        url: `${process.env.REACT_APP_API_URL}change-password-mail`,
        data: {
          email: email,
          password: password,
        },
  ...
  const handleSubmit = async (event) => {
    ...
    if (!values.new_password.trim()) {
      ...
      err.new_password = "Vui lòng nhập mật khẩu mới từ 8 ký tự trở lên";
    }
